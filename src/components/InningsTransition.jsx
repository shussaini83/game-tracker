export default function InningsTransition({ state, overRuns, overWickets, onContinue }) {
  const inn1 = state.innings[0];
  const score = inn1 ? inn1.overs.reduce((s, o) => s + overRuns(o.balls), 0) : 0;
  const wickets = inn1 ? inn1.overs.reduce((s, o) => s + overWickets(o.balls), 0) : 0;
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center px-6 text-center">
      ...
      <p className="text-slate-400 mb-6">
        ...
        <span className="text-slate-400 font-semibold">/{wickets}</span>
        {' '}runs
      </p>
